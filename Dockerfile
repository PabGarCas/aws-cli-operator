FROM alpine:3.20.3
MAINTAINER Pablo Garrido Castreño

RUN apk add --no-cache python3 py3-pip py3-kubernetes git curl bash
WORKDIR /tmp
RUN curl -LO https://dl.k8s.io/release/v1.31.1/bin/linux/amd64/kubectl && mv kubectl /bin && chmod +x /bin/kubectl
...
CMD [ "/bin/bash", "-c", "python3 -u /opt/aws-cli-operator/bin/operator.py > /dev/stdout"]